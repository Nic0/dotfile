...
map.dir('t', alias='<down>')
map.dir('s', alias='<up>')
map.dir('c', alias='<left>')
map.dir('r', alias='<right>')
...
map('<C-D>', 'T', fm.move(down=0.5, pages=True))
map('<C-U>', 'S', fm.move(up=0.5, pages=True))
...
map('xt', fm.search(order='tag'))
map('xc', fm.search(order='ctime'))
map('xm', fm.search(order='mimetype'))
map('xs', fm.search(order='size'))
map('x<bg>', fm.hint('*c*time *m*imetype *s*ize *t*ag  *w*:rename'))
...
map('o', fm.open_console('open_with '))
...
map.merge(global_keys)
map.merge(vim_aliases)
map('S', wdg.task_move(0))
map('T', wdg.task_move(-1))
